//! End-to-end check: connect to nperfd, ask it to record a target
//! pid for a few seconds, count the samples + image / thread events
//! that come back. Lets us validate the full daemon → client → parser
//! → SampleSink path without touching nperf-live.
//!
//! Usage:
//!
//!     # in one terminal: start the daemon (root)
//!     sudo ./target/release/nperfd --socket /tmp/nperfd-test.sock
//!
//!     # in another terminal: run the demo against a same-uid pid
//!     cargo run --example record_pid -p nperfd-client -- \
//!         --pid <PID> --duration 5 --socket /tmp/nperfd-test.sock
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use nperfd_client::{drive_session, RemoteOptions};

    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "nperfd_client=info,record_pid=info".into()),
        )
        .init();

    let mut pid: Option<u32> = None;
    let mut duration_secs: u64 = 5;
    let mut socket = "/tmp/nperfd.sock".to_string();
    ...
    impl SampleSink for CountingSink {
        ...
        fn on_wakeup(&mut self, _: nerf_mac_capture::WakeupEvent<'_>) {
            self.wakeups += 1;
        }
    }
...
fn main() {
    eprintln!("nperfd-client is macOS-only");
}

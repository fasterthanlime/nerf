...
// nerf vendor note: replaced rand-based random i64 with a SystemTime+pid
// hash to avoid pulling in rand 0.10 (whose chacha20 transitive dep is
// pre-release only). See use site below.
fn nonce_i64() -> i64 {
    ...
    (nanos.wrapping_mul(0x9E3779B97F4A7C15).wrapping_add(pid) as i64).abs()
}
...
impl OsIpcReceiver {
    ...
    fn register_bootstrap_name(&self) -> Result<String, MachError> {
        ...
        unsafe {
            ...
            let mut name;
            loop {
                name = format!("{}{}", BOOTSTRAP_PREFIX, nonce_i64());

...
use crate::types::{Bitness, Endianness, Inode};
use crate::utils::{get_major, get_minor, HexValue, StableIndex};
...
impl std::fmt::Debug for LoadHeader {
    fn fmt(&self, fmt: &mut std::fmt::Formatter) -> std::fmt::Result {
        fmt.debug_struct("LoadHeader")
            ...
            .field(
                "flags",
                &Perms(self.is_readable, self.is_writable, self.is_executable),
            )
            .finish()
    }
}
...
impl BinaryData {
    ...
    where
        S: From<gimli::EndianReader<gimli::RunTimeEndian, BinaryDataSlice>>
    ...
    pub fn debuglink(&self) -> Option<&[u8]> {
        ...
        let debuglink_length = debuglink
            .iter()
            .position(|&byte| byte == 0)
            .unwrap_or(debuglink.len());

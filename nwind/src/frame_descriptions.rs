...
impl<E: Endianity> FrameDescriptions<E> {
    ...
    fn load_with_options(builder: FrameDescriptionsBuilder<E>) -> Option<Self> {
        ...
        if debug_frame.is_some() && builder.load_debug_frame {
            ...
            debug!(
                "Loaded {} FDEs from .debug_frame for '{}' in {}ms",
                debug_descriptions.len(),
                binary.name(),
                get_ms(elapsed)
            );
        } else {
        ...
        let want_to_load_eh_frame = builder.load_eh_frame == LoadHint::Always
            || (builder.load_eh_frame == LoadHint::WhenNecessary && eh_frame_hdr.is_none());
        ...
        if eh_frame.is_some() && want_to_load_eh_frame {
            ...
            debug!(
                "Loaded {} FDEs from .eh_frame for '{}' in {}ms",
                eh_descriptions.len(),
                binary.name(),
                get_ms(elapsed)
            );
    ...
    fn load_eh_frame_hdr(
        ...
        let eh_frame_hdr = match eh_frame_hdr.parse(&bases, mem::size_of::<usize>() as u8) {
            ...
            Err(error) => {
                warn!(
                    "Failed to load .eh_frame_hdr for {}: {}",
                    binary.name(),
                    error
                );
                return None;
    ...
    fn load_section<R, U>(
        ...
        name: &str,
        section: &U,
    ) -> RangeMap<FrameDescriptionEntry<R>>
    where
    ...
    pub fn find_unwind_info<'a>(
        ...
        if let Some((bases, eh_frame_hdr)) = self.eh_frame_hdr.as_ref() {
            ...
            match fde {
                ...
                Err(error) => {
                    debug!(
                        "FDE not found in .eh_frame_hdr for 0x{:016X}: {}",
                        absolute_address, error
                    );
                }
            }
        }
...
impl<'a, E: Endianity> UnwindInfo<'a, E> {
    ...
    pub fn initial_absolute_address(&self) -> u64 {
        ((self.absolute_address as i64) + ((self.initial_address as i64) - (self.address as i64)))
            as u64
    }
    ...
    #[inline]
    pub fn each_register<F: FnMut((Register, &RegisterRule<DataReader<E>>))>(
        &self,
        mut callback: F,
    ) {
        match self.kind {
    ...
    pub fn cache_into(&self, unwind_cache: &mut UnwindInfoCache) {
        ...
        let cache = unwind_cache
            .cache
            .get_or_insert_with(|| LruCache::new(NonZeroUsize::new(4096).unwrap()));
        ...
        let info = CachedUnwindInfo {
            rules,
            cfa,
            initial_address: self.initial_address,
            address: self.address,
            is_signal_frame: self.is_signal_frame,
        };
...
#[derive(Default)]
pub struct DynamicFdeRegistry<E>
where
    E: Endianity,
{
...
impl DynamicFdeRegistry<gimli::NativeEndian> {
    pub unsafe fn register_fde_from_pointer(&mut self, fde: *const u8) {
        ...
        let fde_map =
            FrameDescriptions::<gimli::NativeEndian>::load_section(&bases, "<dynamic>", &section);

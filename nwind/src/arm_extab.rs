...
#[test]
fn test_regs_iter() {
    fn to_vec<I: Iterator<Item = Reg>>(iter: I) -> Vec<u8> {
        iter.map(|reg| reg.0).collect()
    }
...
fn test_fp_regs_iter() {
    fn to_vec<I: Iterator<Item = FpReg>>(iter: I) -> Vec<u8> {
        iter.map(|reg| reg.0).collect()
    }
...
fn search(exidx: &[IndexEntry], exidx_base: u32, address: u32) -> Option<usize> {
    ...
    loop {
        ...
        let entry_function_address =
            exidx_offset(exidx_base, mid as u32, entry.offset_to_function());
...
impl<I: Iterator<Item = u8>> Iterator for Decoder<I> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let instruction = if opcode & EXTAB_OP_ADD_VSP_MASK == EXTAB_OP_ADD_VSP {
            ...
            let offset_1 = if opcode & EXTAB_OP_POP_VFP_REGS_ARG_OFFSET_MASK == 0 {
                16
            } else {
                0
            };
...
fn test_decode_pop_r14() {
    assert_eq!(
        ...
        (vec![Instruction::PopRegs(RegMask(1 << 14))], 2, None)
    );
}
...
fn test_decode_pop_r4() {
    assert_eq!(
        decode_to_vec(&[0xa0]),
        (vec![Instruction::PopRegs(RegMask(1 << 4))], 1, None)
    );
}
...
fn test_decode_pop_r3() {
    assert_eq!(
        ...
        (vec![Instruction::PopRegs(RegMask(1 << 3))], 2, None)
    );
}
...
fn test_decode_pop_d8() {
    assert_eq!(
        ...
        (vec![Instruction::PopFpRegs(FpRegMask(1 << 8))], 2, None)
    );
}
...
fn test_decode_pop_d1() {
    assert_eq!(
        ...
        (vec![Instruction::PopFpRegs(FpRegMask(1 << 1))], 2, None)
    );
}
...
fn test_decode_set_vsp_to_r7() {
    assert_eq!(
        decode_to_vec(&[0x97]),
        (vec![Instruction::VspSet(Reg(7))], 1, None)
    );
}
...
fn test_decode_add_vsp_4() {
    assert_eq!(
        decode_to_vec(&[0x00]),
        (vec![Instruction::VspAdd(4)], 1, None)
    );
}
...
fn test_decode_add_vsp_256() {
    assert_eq!(
        decode_to_vec(&[0x3f]),
        (vec![Instruction::VspAdd(256)], 1, None)
    );
}
...
fn test_decode_add_vsp_uleb() {
    assert_eq!(
        ...
        (vec![Instruction::VspAdd(0x204 + (0x98765 << 2))], 4, None)
    );
}

#[test]
fn test_decode_finish() {
    assert_eq!(decode_to_vec(&[0xb0]), (vec![Instruction::Finish], 1, None));
}
...
pub enum Error {
    ...
    MemoryUnaccessible { address: u32 },
...
fn test_bytecode_iter_inline_bytecode_with_offset_and_with_bytecode() {
    ...
    let vec: Vec<u8> =
        BytecodeIter::new(LittleEndian::read_u32(inline_bytecode), 1, bytecode).collect();
...
fn get_bytecode_iter<'a>(
    ...
    extab_base: u32,
    extab: &'a [u8],
) -> Result<BytecodeIter<'a>, Error> {
    if entry.value() & EXIDX_INLINE_MASK != 0 {
        let value = entry.value() & !EXIDX_INLINE_MASK;

        debug!(
            "Entry for 0x{:08X} (index: {}) is defined inline: 0x{:08X}",
            address, index, value
        );
        return Ok(BytecodeIter::new(value, 1, &[]));
    }

    let extab_address = exidx_offset(exidx_base, index as u32, entry.value())
        + mem::size_of_val(&entry.raw_offset_to_function) as u32;

    debug!(
        "Entry for 0x{:08X} (index: {}) is defined in .ARM.extab at: 0x{:08X}",
        address, index, extab_address
    );
    ...
    if extab_entry & EXTAB_HEADER_MODEL_MASK == 0 {
        ...
        debug!(
            "0x{:08X} uses the generic model with personality at offset 0x{:08X}",
            extab_address, personality_offset
        );
        ...
        // ARM compact model.
        let personality_routine = (extab_entry & EXTAB_HEADER_PERSONALITY_ROUTINE_MASK)
            >> EXTAB_HEADER_PERSONALITY_ROUTINE_SHIFT;

        debug!(
            "0x{:08X} uses the ARM compact model with personality equal to {}",
            extab_address, personality_routine
        );
        ...
        match personality_routine {
            ...
            _ => Err(Error::UnsupportedPersonality(personality_routine as u8)),
        }
    }
}
...
fn find_entry(
    ...
    let exidx: &[IndexEntry] = unsafe {
        slice::from_raw_parts(
            exidx.as_ptr() as *const IndexEntry,
            exidx.len() / mem::size_of::<IndexEntry>(),
        )
    };
    ...
    let function_end = if index + 1 < exidx.len() {
        exidx_offset(
            exidx_base,
            index as u32 + 1,
            exidx[index + 1].offset_to_function(),
        )
    } else {
        !0
    };
...
pub fn unwind_from_cache<M>(
    ...
    regs: &mut Regs,
    address: u32,
) -> Option<Result<Option<u32>, Error>>
where
    M: MemoryReader<arm::Arch>,
{
    ...
    M: MemoryReader<arm::Arch>,
{
    ...
    let (index, entry, function_range) = match find_entry(
        exidx,
        exidx_base,
        if is_first_frame { address } else { address - 1 },
    ) {
        Some(result) => result,
        None => {
    ...
    *initial_address = Some(function_start);
    ...
    if entry.value() == EXIDX_CANTUNWIND {
        debug!(
            "Entry for 0x{:08X} (index: {}) doesn't support unwinding",
            address, index
        );
        return Err(Error::EndOfStack);
    }
...
fn test_decode_everything_from_a_binary() {
    ...
    for (index, entry) in exidx.iter().enumerate() {
        ...
        let function_start = exidx_offset(exidx_base, index as u32, entry.offset_to_function());
        let iter =
            get_bytecode_iter(function_start, index, entry, exidx_base, extab_base, extab).unwrap();

...
impl<'a, T> Iterator for RegsIter<'a, T>
where
    T: Copy + Into<u64>,
{
    type Item = (u16, T);
        ...
        impl ::std::fmt::Debug for $regs_ty {
            fn fmt(&self, fmt: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
                ...
                for &register in $regs_array {
                    let name = $arch::register_name_str(register)
                        .map(|name| Cow::Borrowed(name))
                        .unwrap_or_else(|| format!("{}", register).into());
        ...
        impl Registers for $regs_ty {
            ...
            fn get(&self, register: u16) -> Option<Self::RegTy> {
                ...
                let value = if cfg!(debug_assertions) {
                    ...
                    unsafe { *self.as_slice().get_unchecked(register as usize) }
                };
            ...
            fn contains(&self, register: u16) -> bool {
                ...
                debug_assert!(
                    register < (std::mem::size_of_val(&self.mask) * 8) as _,
                    "Out of range register number: {}",
                    register
                );
            ...
            fn append(&mut self, register: u16, value: Self::RegTy) {
                ...
                if cfg!(debug_assertions) {
                    ...
                    unsafe {
                        *self.as_slice_mut().get_unchecked_mut(register as usize) =
                            value as $reg_ty;
                    }
                }
            }
